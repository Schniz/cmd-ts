	...
	return new Parser(async function* () {
		...
		for (const genInfo of activeGenerators) {
			if (!genInfo.completed) {
				let state = await genInfo.gen.next(["continue", false]);

		const collectedErrors: ParsingError[] = [];			// Process any effects that the generator yields during completion
			while (!state.done) {
				switch (state.value.key) {
					case "break": {
						yield* effects.break(...state.value.payload);
						...
						// Collect errors instead of breaking immediately
						collectedErrors.push(...state.value.payload);
						state = { done: true, value: null } as any;
						break;						break;
					}
					case "consume": {
						const consumed = yield* effects.consume(...state.value.payload);
					...
					case "continue": {
						const shouldContinue = yield* effects.continue();
						state = await genInfo.gen.next(["continue", shouldContinue]);
						break;
					}
				}
			}
		...
		// If we collected any errors, break with all of them
		if (collectedErrors.length > 0) {
			yield* effects.break(...collectedErrors);
			return results as { [K in keyof T]: Parser.Type<T[K]> };
		}