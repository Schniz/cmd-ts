...
import { positional, option } from "../src";
...
describe("combinator: all", () => {
	...
	test("doesn't yield in options", async () => {
		const p1 = positional({ displayName: "positional1" });
		const o = option({ long: "option" });
		const p2 = positional({ displayName: "positional2" });
		const parser = all([p1.parser3, p2.parser3, o.parser3]);
		const argv = ArgvItem.normalize(["pos1", "--option", "value", "pos2"]);
		const result = await parse(parser, argv);
		expect(result).toEqual<typeof result>({
			errors: [],
			remainingArgv: [],
			result: {
				value: ["pos1", "pos2", "value"],
			},
		});
	});
});
